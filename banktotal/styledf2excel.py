...
def style_df(df):
    ...
    sf.set_column_width(columns=sf.columns, width=22)
...
def style_df_all(df):
    ...
    # Change the columns width and the rows height
    sf.set_column_width(columns=sf.columns, width=13)
    sf.set_column_width(columns=['transDate', 'description', 'remark', 'amountMoney'], width=25)
    ...
    invalid_style = Styler(bg_color=utils.colors.red, font_color=utils.colors.white, **defaults)
    valid_style = Styler(bg_color=utils.colors.green, font_color=utils.colors.black, **defaults)
